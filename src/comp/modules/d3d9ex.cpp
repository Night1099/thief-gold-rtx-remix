...
#pragma region D3D9Device
	...
	HRESULT d3d9ex::D3D9Device::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
	{
		...
		if (auto* w = worldrep_render::get()) w->on_reset();
		if (auto* l = engine_lights::get()) l->reset();
	...
	HRESULT d3d9ex::D3D9Device::Present(CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
	{
		...
		// ThiefGold: the engine leaks the raw device into its display globals and
		// renders through them, bypassing this wrapper. Re-point the globals at
		// the wrapper every frame (no-op once patched).
	...
	HRESULT d3d9ex::D3D9Device::Clear(DWORD Count, CONST D3DRECT* pRects, DWORD Flags, D3DCOLOR Color, float Z, DWORD Stencil)
	{
		...
		// ThiefGold: a z-only clear mid-scene marks the start of the engine's
		// HUD overlay pass (frame-start clears also carry D3DCLEAR_TARGET).
		// The ray-traced scene must be complete before the first UI draw, so
		// worldrep + lights submit here rather than at EndScene.
		if ((Flags & D3DCLEAR_ZBUFFER) && !(Flags & D3DCLEAR_TARGET))
		{
			submit_scene_to_remix();
			if (auto* u = unproject::get()) u->on_overlay_clear();
		}
	...
	HRESULT d3d9ex::D3D9Device::DrawPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, UINT PrimitiveCount, CONST void* pVertexStreamZeroData, UINT VertexStreamZeroStride)
	{
		...
		// ThiefGold: when worldrep submission is active, suppress the engine's
		// per-frame clipped world fans (identified by the portal-flush return
		// address) — they are replaced by stable whole-polygon geometry at
		// EndScene. Object/particle DPUP draws fall through to unproject.
		if (auto* w = worldrep_render::get(); w && w->enabled() && !shared::globals::imgui_is_rendering)
		{
			if (w->is_world_dpup(reinterpret_cast<uintptr_t>(_ReturnAddress()))) {
				return S_OK;
			}
		}

		// ThiefGold: world geometry arrives here as pre-transformed RHW triangle
		// fans — reconstruct world-space geometry for Remix. UI and unknown
		// layouts fall through to the raw device untouched.
		if (unproject::is_initialized() && !shared::globals::imgui_is_rendering)
		{
			HRESULT hr = S_OK;
			if (unproject::get()->on_draw_primitive_up(m_pIDirect3DDevice9, PrimitiveType, PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride, &hr)) {
				return hr;
			}
		}

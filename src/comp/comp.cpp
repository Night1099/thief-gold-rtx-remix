...
namespace comp
{
	...
	void main()
	{
		...
		// ThiefGold: RHW world-space reconstruction (objects + fallback)
		shared::common::loader::module_loader::register_module(std::make_unique<unproject>());

		// ThiefGold: worldrep whole-polygon submission (stable-hash world geometry)
		shared::common::loader::module_loader::register_module(std::make_unique<worldrep_render>());

		// ThiefGold: engine light table -> Remix path-traced sphere lights
		shared::common::loader::module_loader::register_module(std::make_unique<engine_lights>());
